...
def set_up_area_weightings(modelname, regrid_on=False, calculate_land = True):
    """ Needs docstring """
...
def calculate_area_weightings(modelname, regrid_on=False, calculate_land=True):
    """ Needs docstring """

    # Get data
    ds = set_up_area_weightings(modelname, regrid_on, calculate_land)
...
def get_global_mean_timeseries(ds, modelname, average_type, latname='lat', lonname='lon'):
    if average_type=='global':
        ds_area_weights = calculate_area_weightings(modelname,calculate_land=False)
    else:
    ...
    ds_global = ds_weighted.sum(dim=(latname,lonname))
